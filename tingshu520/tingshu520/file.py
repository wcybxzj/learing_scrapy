...
class FilePipeline(MediaPipeline):
    ...
    def from_settings(cls, settings):
        cls.EXPIRES = settings.getint('FILE_EXPIRES', 30)
    ...
    def media_downloaded(self, response, request, info):
        ...
        filename = self.get_my_file_name(request,response)
    ...
    def file_key(self, url):
        ...
        return '%s/%s' % (time.strftime('%Y-%m-%d',time.localtime(time.time())),file_guid)

    def get_my_file_name(self,request,response):
        import urlparse
        info = urlparse.urlparse(request.url)
        d = dict([x.split("=") for x in info.query.split("&")])
        return str(d['it']) + '.f4v'
